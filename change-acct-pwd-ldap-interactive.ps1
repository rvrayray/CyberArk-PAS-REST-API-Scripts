...
$accountID = Read-Host -Prompt "Enter the Account ID that needs the password changed"
$changeAccountPassword = "$baseUrl/Accounts/$accountID/Change"
...
try {
    ...
    Write-Host "Successfully changed the password for Account ID $accountID."
} catch {
    Write-Host "Failed to change the account password. Please check the Account ID and try again."
    exit
}
